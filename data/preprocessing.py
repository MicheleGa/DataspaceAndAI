...
def parseargs():
    ...
    parser.add_argument('--nest', default='False', type=lambda x: bool(strtobool(x)), help='enable nested JSONs or not')
...
if __name__ == "__main__":
    ...
    for idx, db_name in enumerate(database_names):
        
        # Read CSV/TSV files from the specified directory
        final_formats = process_directory(f'{db_name}')
    
        if args.nest:
            # Apply initial nesting logic
            final_formats = create_nested_json(final_formats)
        
            # Simplify single-key nested dictionaries recursively
            final_formats = simplify_single_key_nesting(final_formats)

        # Output the final JSON file and set an order by imposing an index to the filename
        output_filename = f'{idx + 1}_{db_name}.json'
