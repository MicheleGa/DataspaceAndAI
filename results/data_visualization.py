...
def plot_different_llms_semantic_similarity(csv_file_paths, save_path='./', output_file_name='similarity_graph.png'):
    """
    ...
    # N.B.: even if legend2 variable is not used in the code, it is reported in the plot, do not cancel
...
def plot_similarity_heatmaps(csv_paths, save_path='./', output_file_name='similarity_graph.png'):
    """
    ...
    try:
        ...
        for _, row in df_base.iterrows():
            json_x = row['x']
            json_y = row['y']
            similarity = row['semantic'] # We are only intereste into semantic similarity
        ...
        print(f"Error: Missing expected column in '{base_csv_path}'. "
              f"Please ensure 'x', 'y', and 'semantic' columns exist. Details: {e}")
        return
    except Exception as e:
    ...
    plt.title('Pairwise Semantic Similarities (Initial JSONs) - Base Data', fontsize=14)
    ...
    plt.savefig(output_file_path, dpi=300)
    print(f"Saved base semantic similarity heatmap to base_{output_file_path}")

    # --- Step 3: Process and plot LLM-specific heatmaps ---
    # For each subsequent CSV (representing an LLM's results), we create a new heatmap. 
    for i, llm_csv_path in enumerate(csv_paths[1:]):
        ...
        try:
            ...
            plt.title(f'Semantic Similarities with Final Harmonized Schema ({llm_name})', fontsize=14)
            plt.xlabel('JSONs', fontsize=12)
            plt.ylabel('JSONs', fontsize=12)
            plt.xticks(rotation=45, ha='right')
            plt.yticks(rotation=0)
            ...
            plt.savefig(output_file_path, dpi=300)
            print(f"Saved {llm_name} semantic similarity heatmap to {llm_name}_{output_file_name}")
...
if __name__ == "__main__":
    ...
    if args.plot_different_llms:
        ...
        if args.plot_llms_heatmaps:
            plot_different_llms_semantic_similarity(
